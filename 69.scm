(define-syntax test-lset-eq?
  (syntax-rules ()
    ((test-lset-eq? expected-value expr)
     (check-predicate
      (lambda (a b) (lset= eq? a b))
      expected-value
      expr))))

(define-syntax test-lset-equal?
  (syntax-rules ()
    ((test-lset-equal? expected-value expr)
     (check-predicate
...
(define-syntax test
  (syntax-rules ()
    ((_ expected-value expr)
     (check-equal? expr expected-value))))
...
(define (lset= predicate as bs)
  (define (one-way as bs)
    (let outer ((as as))
      (or (null? as)
          (let inner ((bs bs))
            (cond ((null? bs) #f)
                  ((predicate (car as) (car bs)) (outer (cdr as)))
                  (else (inner (cdr bs))))))))
  (and (one-way as bs) (one-way bs as)))
