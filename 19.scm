...
(test-begin "Creating time structures")
...
(test-begin "Testing time resolutions")
...
(test-begin "Time comparisons (time=?, etc.)")
...
(test-begin "TAI-UTC Conversions")
...
(test-begin "TAI-Date Conversions")
...
(test-begin "Date-UTC Conversions")
...
(test-begin "TZ Offset conversions")
...
(test-begin "date->string")
(let ((d (make-date 0 1 2 3 4 5 2006 0)))
  (test-assert (string=? " 3" (date->string d "~k")))
  (test-assert (string=? " 3" (date->string d "~l")))
  (test-assert (string=? "2006-05-04T03:02:01" (date->string d "~5")))
  (test-assert (string=? "2006-05-04T03:02:01Z" (date->string d "~4")))
  (test-assert (string=? "03:02:01" (date->string d "~3")))
  (test-assert (string=? "03:02:01Z" (date->string d "~2"))))
(test-end)
    ...
    (lambda (date)
      (let ((p (open-output-string)))
        ...
        (test-assert
          (equal?
            (date->string
              (make-date 0 0 0 0 (caddr date) (cadr date) (car date) 0) "~V")
            (cdddr date)))))
    dates))
(test-end)
