...
(test-equal #f (vector= = '#(+nan.0) '#(+nan.0)))
(test-equal #f (let ((nan '+nan.0)) (vector= = (vector nan) (vector nan))))
(test-equal #f (let ((nanvec '#(+nan.0))) (vector= = nanvec nanvec)))
...
(test-equal
    3 (vector-fold (lambda (ctr n) (if (even? n) (+ ctr 1) ctr)) 0 vec))
...
(vector-unfold!
 (lambda (i x y) (values (+ i x y) (+ x 1) (+ x 1))) vu2 1 4 0 0)
...
(vector-unfold-right!
 (lambda (i x y) (values (+ i x y) (+ x 1) (+ x 1))) vur2 1 4 0 0)
